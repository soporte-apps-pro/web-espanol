...
function weekStatusClass(slot) { if (slot.status === "closed") return "blocked"; if (slot.status === "confirmed") return "confirmed"; if (["held","payment_review"].includes(slot.status)) return "pending"; if(!slot.googleCalendarCheckedAt?.toMillis?.())return "pending"; if(slot.googleCalendarBlocked===true)return "blocked"; return googleCleared(slot)?"available":"pending"; }
...
function googleStatus(slot) { if (!slot.googleCalendarCheckedAt?.toMillis?.()) return "Google Calendar: pendiente de primera revisión"; if (slot.googleCalendarBlocked === true) return "Google Calendar: detectó un cruce; no se muestra al estudiante"; if (googleCleared(slot)) return "Google Calendar: libre y verificado"; return "Google Calendar: necesita una nueva revisión"; }
function slotLabel(slot) { if(slot.status!=="available")return statusLabel(slot.status); if(googleCleared(slot))return "Disponible para reservar"; return slot.googleCalendarCheckedAt?.toMillis?.() && slot.googleCalendarBlocked===true ? "Bloqueado por calendario" : "Pendiente de revisión"; }
...
function slotCard(slot) {
  ...
  return `<details class="private-card private-slot-card" data-slot-id="${escapeHtml(slot.id)}" ${request?.status === "payment_review" ? "open" : ""}>
    <summary class="group-card-head"><div><h3>${escapeHtml(formatColombia(slot.startAt))}</h3><span class="muted">${slot.durationMinutes||50} minutos · hora Colombia<br>${escapeHtml(googleStatus(slot))}</span></div><span class="pill">${escapeHtml(slotLabel(slot))}</span></summary>
