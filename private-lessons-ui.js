...
import { mountPrivateTopups } from "./private-topups-ui.js?v=20260916-terms-1";
...
export function mountPrivateLessons(root, app, admin = false) {
  ...
  async function execute(data, form) {
    ...
    try {
      ...
      if(form?.elements.termsMode){if(data.action==="configure"){form.elements.termsMode.value=data.terms?"custom":"general";for(const key of ["durationMinutes","packageQuantity","packageAmountUsd","paymentUrl","paymentNote"])form.elements[key].value=data.terms?.[key]??({durationMinutes:50,packageQuantity:1}[key]??"");}toggleTerms(form);}
